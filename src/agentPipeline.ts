...
export async function runMultiAgentSqlPipeline(
  ...
  const addLog = (agent: string, status: AgentLog["status"], msg: string, details?: any) => {
    logs.push({ agentName: agent, status, message: msg, timestamp: new Date().toISOString(), details });
  };
...
Return in JSON format:
{
  ...
  return {
    ...
    healingAttempts,
    confidenceScore,
    explanation: { businessSummary, steps, technicalDetails },
    security: { isSafe, detectedRisks, userRole, appliedGuardrails },
    optimization: { estimatedCost, indexRecommendations, improvements },
    visualization: { chartType, xAxisKey, yAxisKey, title: chartTitle },
    logs
  };
}
